...
> **你永远读不完所有论文——你本来也不必读完。**
>
> 真正的能力是判断：*先读哪一篇。* 而这种判断没有放之四海的标准。在*你*的领域里，什么样的
> 论文最该先读，只有你自己说得清——是某种检测技术、模型系统、临床阶段，还是某个数据集。
> LitTriage 把这种判断变成一套**由你设计**的评分算法，再据此为你的文献排序。于是第一天，
> 你打开的就是最该读的那篇——而不是 PubMed 的第 7 页。
>
> 它是一个你用的工具——更是一套你让它属于自己的排序算法。
...
> **设计笔记 —— 把评分变成你自己的。** 这是一个值得借鉴的思路，无论你在哪个领域：
> 一个好的排序算法，*骨架*可以跨学科通用——但*评分维度*该由你自己来定。LitTriage
> 恰好以临床证据来打分，但那只是一种配方。比如面向转化医学，你会把**检测技术/方法学**
> （HPLC-MS、ELISA、免疫组化等）纳入「方法匹配」，把**模型系统**（体外→动物→人）单列
> 为一个维度，并让**临床成熟度抬高分数**——一个已进入临床的生物标志物，不该输给只在
> 培养皿里验证过的。每个领域都有自己对*「哪篇文献最该先读」*的判断；把那几个维度命名
> 出来，同一套机器就能按你的方式给你的文献排序。
> 详见 [`references/02_scoring_translational.md`](references/02_scoring_translational.md)，那是一个可以直接复制改写的范例。
>
> 🎓 **如果你想为自己的研究领域设计这套评分算法，就从这里开始。**
